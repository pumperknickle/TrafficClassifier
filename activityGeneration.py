...
def convert_sequence(sequence, all_signatures):
  if len(sequence) == 0:
    return []
  if len(sequence) < 2:
    return [sequence]
  sortedKeys = sorted(all_signatures.keys(), reverse=True)
  for i in sortedKeys:
    prefix = sequence[:i]
    signatures = all_signatures[i]
    for signature in signatures:
      if matches(prefix, signature):
        return [signature] + convert_sequence(sequence[i:], all_signatures)
  return [sequence[:1]] + convert_sequence(sequence[1:], all_signatures)
...
def generateWithTransitions(transitions, n):
  all_gen = []
  for num in range(n):
    generated = []
    generatedSigs = []
...
def createTransitions(sequences, all_signatures):
  converted_sequences = []
  for sequence in sequences:
    converted_sequences.append(convert_sequence(sequence, all_signatures))
  transitions = dict()
...
def generateWithSignatures(sequences, all_signatures, n):
  converted_sequences = []
  for sequence in sequences:
    converted_sequences.append(convert_sequence(sequence, all_signatures))
  transitions = dict()
